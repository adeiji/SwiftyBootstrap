...
open class GRMessageCard: GRBootstrapElement {
    ...
    public func draw (message: String, title:String, buttonBackgroundColor:UIColor = .black, superview: UIView, buttonText: String = "Okay", cancelButtonText:String? = nil, isError: Bool = false) {
        ...
        let cancelButton = Style.largeButton(with: cancelButtonText ?? "", backgroundColor: .lightGray, fontColor: .darkGray)
        ...
        .addRow(columns: [
            ...
            Column(cardSet: Style.label(withText: title, fontName: .allBold, superview: nil, color: UIColor.black.dark(.white))
                .toCardSet()
                .margin.left(30)
                .margin.right(30)
                .margin.top(30),
